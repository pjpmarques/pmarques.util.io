...
/*
 * This implementation is based on Apache' Harmony BufferedReader class. See:
 * http://harmony.apache.org/
 *
 * svn checkout http://svn.apache.org/repos/asf/harmony/enhanced/java/trunk
 *
 * Code adapted by Paulo Marques (paulo.marques@feedzai.com).
 */
...
/**
 * This class defines a BoundedBufferedReader, compatible with Java's BufferedReader
 * where readLine() either returns a complete line or a line up to a certain number of characters.
 *
...
public class BoundedBufferedReader extends Reader {
    ...
    public BoundedBufferedReader(Reader in, int size) {
        super(in);
        if (size <= 0) {
            throw new IllegalArgumentException("Invalid size passed to BoundedBufferReader (<=0)");
        }
        this.in = in;
        buf = new char[size];
    }
    ...
    public void mark(int markLimit) throws IOException {
        ...
        synchronized (lock) {
            if (isClosed()) {
                throw new IOException("Attempting to mark() an already closed reader");
            }
    ...
    public int read() throws IOException {
        synchronized (lock) {
            if (isClosed()) {
                throw new IOException("Attempting to read() an already closed reader"); //$NON-NLS-1$
            }
    ...
    public String readLine(int maxChars) throws IOException {
        ...
        synchronized (lock) {
            if (isClosed()) {
                throw new IOException("Attempting to read() an already closed reader");
            }
            ...
            for (int charPos = pos; charPos < end; charPos++) {
                ...
                if (ch!='\r' && ch!='\n') {
                    ...
                    if (charsInLine > maxChars)
                        throw new LineToLongException("Parsed a line bigger than " + maxChars);
                }
                // }
    ...
    public boolean ready() throws IOException {
        synchronized (lock) {
            if (isClosed()) {
                throw new IOException("Attempting to check if a reader will block on an already closed reader"); //$NON-NLS-1$
            }
    ...
    public void reset() throws IOException {
        synchronized (lock) {
            if (isClosed()) {
                throw new IOException("Attempting to reset() an already closed reader"); //$NON-NLS-1$
            }
            if (mark == -1) {
                throw new IOException("Mark is currently -1 when calling reset() on a reader");
            }
            pos = mark;
        }
    }
    ...
    public long skip(long amount) throws IOException {
        ...
        synchronized (lock) {
            if (isClosed()) {
                throw new IOException("Attempting to skip() bytes on an already closed reader");
            }
            if (amount < 1) {
                return 0;
            }
